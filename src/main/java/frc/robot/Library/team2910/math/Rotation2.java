...
public final class Rotation2 implements Interpolable<Rotation2>, Serializable {
   ...
   * @param normalize Whether or not to normalize the x and y coordinates. Should be set to true if
   *     it is uncertain if the point is on the unit circle.
   */
  public Rotation2(double x, double y, boolean normalize) {
    if (normalize) {
      double length = Math.sqrt(x * x + y * y);

      // If the length is so small that we are unsure if the point has a direction, default to an
      // angle of 0 degrees.
    ...
    // Tangent has special cases if the cosine of the angle is 0 (Straight up on the unit circle or
    // straight down).
  ...
  public Rotation2 rotateBy(Rotation2 other) {
    // This is implemented as a rotation matrix. The rotation "this" is rotated by a rotation matrix
    // created by the
    // rotation "other".
   ...
   *
   * <p>This is different from {@link #equals} because it also takes into account rotations that are
