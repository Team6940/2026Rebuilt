...
public class Util {
  ...
  /**
   * Normalizing a vector scales it so that its norm is 1 while maintaining its direction. If input
   * is a zero vector, return a zero vector.
   *
  ...
  public static boolean shouldReverse(double goalAngle, double currentAngle) {
    ...
    reversedAngleDifference =
        (reversedAngleDifference > 180) ? 360 - reversedAngleDifference : reversedAngleDifference;
  ...
  /**
   * written as a possible alternative to test logic (when tested in a test class, it produced the
   * exact same outputs as the other method so this is not the problem)
   *
   ...
   */
  public static double[] convertCoordinateSystem(
      CoordinateSystem cSystem, Translation2d translation, double theta) {
   ...
   */
  public static double[] convertCoordinateSystem(
      CoordinateSystem cSystem, double x, double y, double theta) {
  ...
  /**
   * https://stackoverflow.com/a/1167047/6627273 A point D is considered "within" an angle ABC when
   * cos(DBM) > cos(ABM) where M is the midpoint of AC, so ABM is half the angle ABC. The cosine of
   * an angle can be computed as the dot product of two normalized vectors in the directions of its
   * sides. Note that this definition of "within" does not include points that lie on the sides of
   ...
   * @author Joshua Huang
   */
  public static boolean vectorIsWithinAngle(
      Translation2d test, Translation2d A, Translation2d B, Translation2d C, boolean vertical) {
    Translation2d M = vectorInterpolate(A, C, 0.5); // midpoint
    Translation2d m =
        normalize((new Translation2d(M.getX() - B.getX(), M.getY() - B.getY()))); // mid-vector
    Translation2d a =
        normalize((new Translation2d(A.getX() - B.getX(), A.getY() - B.getY()))); // side vector
    Translation2d d =
        normalize(
            (new Translation2d(test.getX() - B.getX(), test.getY() - B.getY()))); // vector to here
    if (vertical) {
      m = vectorInverse(m);
      a = vectorInverse(a);
    }
  ...
  public static boolean vectorIsWithinAngle(
      Translation2d test, Translation2d A, Translation2d B, Translation2d C) {
    return vectorIsWithinAngle(test, A, B, C, false);
  }

  /** Assumes an angle centered at the origin. */
  public static boolean vectorIsWithinAngle(
      Translation2d test, Translation2d A, Translation2d C, boolean vertical) {
  ...
  public static Translation2d vectorInterpolate(
      Translation2d test, final Translation2d other, double x) {
    if (x <= 0) {
      return test;
    } else if (x >= 1) {
      return other;
  ...
  public static Translation2d vectorExtrapolate(
      Translation2d test, final Translation2d other, double x) {
    return new Translation2d(
        x * (other.getX() - test.getX()) + test.getX(),
        x * (other.getY() - test.getY()) + test.getY());
  }
